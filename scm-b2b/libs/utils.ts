...
export function dateformatYYYYMMDDmmss() {
    let date, month, year, minutes, second;
    date = new Date().getDate();
    month = new Date().getMonth() + 1;
    year = new Date().getFullYear();
    ...
    date = date.toString().padStart(2, '0');
    month = month.toString().padStart(2, '0');
    return `${year}${month}${date}${minutes}${second}`;
}

export function viewKorean(price: number) {
    var result = '';
    if (price > 0) {
        var number_price = price + '';
        var hanA = new Array('', '일', '이', '삼', '사', '오', '육', '칠', '팔', '구', '십');
        var danA = new Array('', '십', '백', '천', '', '십', '백', '천', '', '십', '백', '천', '', '십', '백', '천');

        for (let i = 0; i < number_price.length; i++) {
            let str = '';
            let han = hanA[number_price.charAt(number_price.length - (i + 1))];
